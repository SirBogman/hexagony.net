...
import { arrayInitialize } from '../hexagony/Util';
import { colorModes, parseBreakpoint } from '../view/ViewUtil';
import { calculateX, calculateY, cellOffsetY, cellWidth, getHexagonOffsets, OffsetsArray } from './GridShared';
import { GridCell } from './GridCell';
import { GridCellBreakpoint } from './GridCellBreakpoint';
import { GridEdgeConnectors } from './GridEdgeConnectors';
...
import { CodeChangeCallback } from '../view/UndoItem';
...
type CodePanelProps = {
    ...
    instructionPointers: readonly InstructionPointer[] | null;
    grid: string[][];
    onEditHexagonCharacter: CodeChangeCallback;
    onToggleBreakpoint: (i: number, j: number) => void;
    size: number;
    selectedIp: number;
    showArrows: boolean;
    showIPs: boolean;
...
type CodePanelState = {
    focusedI: number | null;
    focusedJ: number | null;
    focusedK: number | null;
}

export class CodePanel extends React.PureComponent<CodePanelProps, CodePanelState> {
    private edgeConnectorsRef: React.RefObject<GridEdgeConnectors> = React.createRef();
    ...
    playEdgeAnimation(edgeTraversal: EdgeTraversal): void {
        if (this.edgeConnectorsRef.current) {
            this.edgeConnectorsRef.current.playEdgeAnimation(edgeTraversal);
        }
    }

    render(): JSX.Element {
        const { colorMode, colorOffset, breakpoints, delay, edgeTransitionMode, instructionPointers,
            onEditHexagonCharacter, onToggleBreakpoint, grid, selectedIp, size } = this.props;
        ...
        const offsets = edgeTransitionMode ? getHexagonOffsets(size) : [[0, 0, 'Center']] as OffsetsArray;
        ...
        if (instructionPointers !== null) {
            [[activeI, activeJ, dir]] = instructionPointers[selectedIp].executionHistory;
        }
