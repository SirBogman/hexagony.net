...
import { applyColorMode, assertDefined, assertNotNull, colorModes, darkColorMode, getControlKey, parseBreakpoint } from '../view/ViewUtil';
...
export class App extends React.Component<IAppProps, IAppState> {
    ...
    private codePanelRef: React.RefObject<CodePanel> = React.createRef();
    ...
    private onEditHexagonCharacter: CodeChangeCallback = (char, codeChangeContext) =>
    ...
    private * getBreakpoints(): Generator<readonly number[]> {
    ...
    private onToggleBreakpoint = (i: number, j: number): void => {
        const id = `${i},${j}`;
    ...
    private playEdgeAnimation = (edgeTraversal: EdgeTraversal): void => {
        const codePanel = this.codePanelRef.current;
        if (codePanel) {
            codePanel.playEdgeAnimation(edgeTraversal);
        }
    };
    ...
    private onStepBack = (): void => {
        ...
        hexagony.edgeTraversals.forEach(this.playEdgeAnimation);
    ...
    render(): JSX.Element {
        const { animationDelay, link, isGeneratedLinkUpToDate, isRunning, sourceCode, selectedIp, typingDirection, userData } = this.state;
        const { hexagony } = this;
            ...
            <>
                <div id="appGrid" className={hexagony ? 'playGrid' : 'editGrid'}>
                    ...
                    <CodePanel
                        ...
                        instructionPointers={hexagony !== null ? hexagony.ips : null}
