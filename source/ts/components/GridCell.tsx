...
import { removeWhitespaceAndDebug } from '../hexagony/Util';
import { CodeChangeCallback } from '../view/UndoItem';
import { assertNotNull } from '../view/ViewUtil';
import { cellHeight } from './GridShared';
...
export class GridCell extends React.PureComponent<GridCellProps> {
    ...
    componentDidUpdate(): void {
        if (this.props.isFocused) {
            const element = assertNotNull(this.inputRef.current, 'inputRef');
    ...
    onInput = (): void => {
        ...
        const element = assertNotNull(this.inputRef.current, 'inputRef');
        const newText = removeWhitespaceAndDebug(element.value) || '.';
        onEditHexagonCharacter(newText, { i, j });
    ...
    render(): JSX.Element {
        const { className, children, delay, isFocused, onBlur, text, x, y } = this.props;
        let innerContent;
